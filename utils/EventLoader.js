...
module.exports = function (client) {
    ...
    for (const { path, data } of files) {
        try {
            // Ignore files meant for utilities like hotReload
            if (!data.name) {
                client.logs.debug(`Skipping ${path} (no name property)`);
                continue;
            }
            ...
            if (!Events[data.name] && !Object.values(Events).includes(data.name)) {
                client.logs.warn(`Possibly invalid event name "${data.name}" - This will never be called unless custom.`);
            }
